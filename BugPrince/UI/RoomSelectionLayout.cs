...
internal class RoomSelectionLayout
{
    ...
    private static float YPos(RowType rowType) =>
        UIConstants.Y_MAIN
        + rowType switch
        {
            RowType.Mid => 0,
            RowType.Top => UIConstants.SCENE_Y_SPACE / 2,
            RowType.Bottom => -UIConstants.SCENE_Y_SPACE / 2,
            _ => throw rowType.InvalidEnum(),
        };
    ...
    public RoomSelectionLayout(int size)
    {
        if (size == 0)
            return;
        if (size < 1 || size > 6)
            throw new System.ArgumentException($"Bad size: {size}");

        if (size <= 3)
            AddRow(RowType.Mid, size);
        else
        {
            int top = (size + 1) / 2;
    ...
    private void AddRow(RowType rowType, int size)
    {
        ...
        {
            RoomSelectionLayoutPosition pos = new() { Index = idx, Pos = new(XPos(i, size), yPos) };

            if (i > 0)
                pos.LeftIndex = idx - 1;
            if (rowType == RowType.Bottom)
                pos.UpIndex = idx - topRowSize;
    ...
    private void MirrorDirections()
    {
        ...
        {
            if (pos.LeftIndex.HasValue)
                positions[pos.LeftIndex.Value].RightIndex = pos.Index;
            if (pos.UpIndex.HasValue)
                positions[pos.UpIndex.Value].DownIndex = pos.Index;
        }
    }
}
