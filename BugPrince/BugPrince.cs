...
public class BugPrinceMod : Mod, IGlobalSettings<GlobalSettings>, ICustomMenuMod
{
    ...
    private static readonly string version =
        PurenailCore.ModUtil.VersionUtil.ComputeVersion<BugPrinceMod>();
    ...
    public override List<(string, string)> GetPreloadNames() =>
        BugPrincePreloader.Instance.GetPreloadNames();
    ...
    public MenuScreen GetMenuScreen(MenuScreen modListMenu, ModToggleDelegates? toggleDelegates)
    {
        ...
        builder.AddHorizontalOption(
            new()
            {
                ...
                Description =
                    "If yes, BugPrince will precompute room choices when entering a scene to save time.",
                Values = ["No", "Yes"],
        ...
        builder.AddHorizontalOption(
            new()
            {
                ...
                Description =
                    "If yes, BugPrince will update rando map mod with transition choices whenever you open the map.",
                Values = ["No", "Yes"],
    ...
    internal static void StartDebugLog() =>
        LogManager.Write("--- BugPrinceDebugLog ---\n", DEBUG_LOG_FILENAME);

    internal static void DebugLog(string message) =>
        LogManager.Append($"{message}\n", DEBUG_LOG_FILENAME);

    // Public API
    public static void AddCostGroupProducer(Mod source, string name, ICostGroupProducer producer) =>
        CostGroup.AddProducer($"{source.Name}-{name}", producer);

    public static void AddSceneSprite(string sceneName, ISprite sprite) =>
        SceneChoiceInfo.AddSceneSprite(sceneName, sprite);
}
