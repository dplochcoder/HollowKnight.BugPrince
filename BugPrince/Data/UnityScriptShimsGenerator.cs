...
{
    internal static void GenerateUnityShims(string root) =>
        GenerateDirectory($"{root}/UnityScriptShims/Scripts/Generated", GenerateUnityShimsImpl);
    ...
    private static void GenerateShimFile(Type type, string dir)
    {
        ...
        string origNs = ns;
        if (ns == "BugPrince.Scripts")
            ns = "";
        else if (ns.ConsumePrefix("BugPrince.Scripts.", out var trimmed))
            ns = trimmed;
