...
{
    internal static string ItemName(this ItemType self) =>
        self switch
        {
    ...
    internal static string PoolName(this ItemType selF) =>
        selF switch
        {
...
internal record LocationData
{
    ...
    {
        if (location is IPinLocationProvider provider)
            return provider.ProvidePinLocation();
        if (location is CoordinateLocation loc)
            return new(loc.sceneName!, loc.x, loc.y);
        if (location is DualLocation dual)
            return DerivePinLocation(dual.falseLocation) ?? DerivePinLocation(dual.trueLocation);
        return null;
    }

    internal void AddInteropData() =>
        Location!.AddInteropData(
            ItemType.PoolName(),
            PinLocationOverride ?? DerivePinLocation(Location!),
            DisplaySource
        );
    ...
    internal LocationDef LocationDef() =>
        new()
        {
    ...
    {
        if (!rs.IsLocationPoolEnabled(LocationPool))
            return LocationState.Skip;

        return ItemType switch
        {
            ItemType.Coin or ItemType.Gem => (rs.AreCostsEnabled && !gs.PoolSettings.Keys)
                ? LocationState.Preplaced
                : LocationState.Randomized,
            ItemType.DiceTotem or ItemType.PushPin => gs.PoolSettings.Relics
                ? LocationState.Randomized
                : LocationState.Preplaced,
    ...
    internal void AddToRequestBuilder(RandomizationSettings rs, RequestBuilder rb)
    {
        ...
        {
            case LocationState.Skip:
                break;
            case LocationState.Preplaced:
                for (int i = 0; i < Count; i++)
                    rb.AddToPreplaced(new(ItemType.ItemName(), Location!.name));
                break;
    ...
    internal void AddVanillaToItemChanger(GenerationSettings gs, RandomizationSettings rs)
    {
        ...
        if (state != LocationState.Preplaced)
            return;
