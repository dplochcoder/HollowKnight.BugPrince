...
internal class OverlaidCostGroupProgressionProvider : ICostGroupProgressionProvider
{
    ...
    internal OverlaidCostGroupProgressionProvider(
        ICostGroupProgressionProvider impl,
        List<string> progressionOverride
    )
    {
        this.impl = impl;
    ...
    public IReadOnlyDictionary<string, string> GetCostGroupsByScene() =>
        impl.GetCostGroupsByScene();

    public bool IsRandomizedTransition(Transition transition) =>
        impl.IsRandomizedTransition(transition);
...
{
    internal static int GetItemCount(this ICostGroupProgressionProvider self, CostType costType) =>
        self.GetCostGroups().Values.Where(g => g.Type == costType).Select(g => g.Cost).Sum();

    internal static bool GetCostGroupByScene(
        this ICostGroupProgressionProvider self,
        string scene,
    ...
    internal static bool GetProgressiveCostByScene(
        this ICostGroupProgressionProvider self,
        string sceneName,
    ...
    )
    {
        costType = default;
        cost = 0;
        if (!self.GetCostGroupByScene(sceneName, out var groupName, out var group))
            return false;
        ...
        {
            if (!self.GetCostGroups().TryGetValue(previousName, out var previousGroup))
                throw new ArgumentException("Bad cost group progression data");

            if (previousGroup.Type == group.Type)
                cost += previousGroup.Cost;
            if (previousName == groupName)
                return true;
        }
