...
internal class NailUpgradesLogicString : IString
{
    ...
    private static bool RandomizeNailUpgrades(Mod mod) =>
        mod.TryGetSettings<RandoPlus.GlobalSettings>(out var settings) && settings.NailUpgrades;

    [JsonIgnore]
    public string Value =>
        (ModHooks.GetMod("RandoPlus") is Mod mod && RandomizeNailUpgrades(mod))
            ? WithRandomizedNailUpgrades
            : WithoutRandomizedNailUpgrades;
