...
internal class MillibelleLocation : ExistingContainerLocation
{
    ...
    protected override void OnLoad() =>
        Events.AddFsmEdit(UnsafeSceneName, FSM_ID, ModifyMillibelle);

    protected override void OnUnload() =>
        Events.RemoveFsmEdit(UnsafeSceneName, FSM_ID, ModifyMillibelle);

    public override ContainerLocation AsContainerLocation() =>
        throw new InvalidOperationException("MillibelleLocation cannot be replaced");

    private const int NUM_HITS = 3;

    private void ModifyMillibelle(PlayMakerFSM fsm)
    {
        List<(string, FlingDirection)> dirs =
        [
            ("Right", FlingDirection.Right),
            ("Left", FlingDirection.Left),
            ("Up", FlingDirection.Any),
